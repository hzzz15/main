...
const TemporaryCareTest = () => {
  ...
  const submitTest = async (selectedTags) => {
    try {
      ...
      // ✅ 결과 페이지로 이동
      navigate("/TemporaryCare_resultPage", { state: { recommendedDogs: data.recommended_dogs } });
    } catch (error) {
  ...
  return (
    <div className="TemporaryCare-container">
      <header className="TemporaryCare-header">
        <div className="TemporaryCare-header-content">
          ...
          />
          <h1>임시 보호 성향 테스트</h1>
          <p className="TemporaryCare-description">우리 강아지와 잘 맞는 보호자를 찾아보세요!</p>
        </div>
      </header>
        ...
        <div className="choices-container">
          {questions[currentIndex].choices.map((choice, index) => (
            <button
              ...
              className={`choice-button ${selectedChoice === choice.text ? "selected" : ""}`}
            >
              {choice.text}
            </button>
          ))}
