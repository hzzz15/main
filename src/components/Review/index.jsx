    ...
    <div className="review-container">
      ...
      <header className="review-header">
        <div className="review-header-content">
          <img src="/reviewicons/back.png" alt="뒤로가기" className="review-back-icon" onClick={() => navigate(-1)} />
          <h1>후기</h1>
        </div>
      </header>
      ...
      <div className="review-tags">
        <button className={`review-tag ${selectedTag === null ? "active" : ""}`} onClick={() => handleTagClick(null)}>
          전체
        </button>
        <button
          ...
          comments.map((comment, index) => (
            <div className="review-card" key={index}>
              <div className="review-content-box">
                ...
                {ratings.length > 0 && ratings[index] !== undefined && <p>평점: {ratings[index].toFixed(1)}</p>}
              </div>
            </div>
          ))
        ) : (
